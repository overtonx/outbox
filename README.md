...
4.  **Ретраи**: поведение при ошибке публикации зависит от `Publisher`.
    -   Для `KafkaPublisher` (по умолчанию) действует режим строгого порядка
        по чекпоинту: любая ошибка доставки останавливает публикацию новых
        событий, пересоздаёт Kafka-продюсер и переигрывает поток заново с
        последнего подтверждённого чекпоинта — точечных ретраев отдельных
        сообщений нет, чтобы не нарушить порядок в пределах `aggregate_id`.
        См. "Устойчивость к сбоям доставки: чекпоинт и replay" ниже.
    -   Для любого другого `Publisher` (не реализующего `OrderedPublisher`)
        используется построчная модель: событие переходит в статус `retry`
        со временем следующей попытки по настраиваемой стратегии backoff,
        либо в `error`, если попытки исчерпаны.
5.  **Dead-Letter**: события, окончательно не поддающиеся публикации,
    переносятся в `outbox_deadletters` — построчным путём при исчерпании
    попыток, либо (для `KafkaPublisher`) как poison-message после
    нескольких подряд неудачных циклов replay, упирающихся в одно и то же
    событие.
...
### Устойчивость к сбоям доставки: чекпоинт и replay

`KafkaPublisher` реализует необязательный интерфейс `OrderedPublisher`, и
`Dispatcher` автоматически переключается на режим строгого порядка, если
`Publisher` его поддерживает (для любой другой реализации — в том числе
собственной — действует построчная модель `retry`/`backoff`, описанная
выше).

Идея: под `enable.idempotence=true` единичная ошибка доставки одного
сообщения посреди партии — не изолированный сбой этого сообщения, а сигнал,
что гарантия порядка для затронутой партиции может быть нарушена (точечный
ретрай мог бы отправить это сообщение позже, чем уже доставленные более
поздние сообщения того же `aggregate_id`). Поэтому вместо построчных
статусов используется единый глобальный чекпоинт — `last_confirmed_id`,
продвигаемый **строго по непрерывному префиксу** подтверждений (ack 1,2,3,5
даёт чекпоинт 3, а не 5), персистируемый в `outbox_checkpoints` не на
каждое подтверждение, а периодически (`WithCheckpointFlushInterval`).

При любой ошибке доставки — фатальной ошибке продюсера
(`kafka.Error.IsFatal()`) или ошибке конкретного сообщения — `Dispatcher`:

1.  останавливает клейминг новых событий;
2.  пересоздаёт Kafka-продюсер (старый флашится и закрывается в фоне);
3.  перечитывает последний персистентный чекпоинт из БД и возобновляет
    публикацию строго с него — переигрывая (replay), а не точечно
    ретраивая, весь диапазон после чекпоинта.

Единственное исключение — poison message: если одно и то же событие сразу
после чекпоинта подряд ломает несколько циклов replay
(`WithPoisonMessageThreshold`, по умолчанию 3), оно принудительно
переносится в `outbox_deadletters`, а чекпоинт продвигается мимо него —
иначе один неотправляемый (например, слишком большой) месседж останавливал
бы весь поток навсегда.

`WithInFlightWindow` ограничивает, насколько далеко вперёд от последнего
подтверждённого чекпоинта можно публиковать, не дожидаясь ack'ов — это и
объём переотправки после replay, и верхняя граница памяти, занимаемой
внутренним трекером подтверждений.
...
-- migrations/0002_checkpoints.sql — только для Publisher'ов в режиме
-- строгого порядка (см. "Устойчивость к сбоям доставки: чекпоинт и replay").
...
Применимо только к `Publisher`, реализующим `OrderedPublisher` (см.
"Устойчивость к сбоям доставки: чекпоинт и replay"):

-   `WithInFlightWindow(int)`: сколько событий вперёд от последнего
    подтверждённого чекпоинта можно опубликовать, не дожидаясь ack'ов. (1000)
-   `WithCheckpointFlushInterval(time.Duration)`: как часто персистентный
    чекпоинт и построчный `status` синхронизируются с текущим watermark.
    (200 мс)
-   `WithPoisonMessageThreshold(int)`: число подряд идущих циклов replay,
    упирающихся в одно и то же событие, после которого оно принудительно
    переносится в `outbox_deadletters`. (3)
